...
x<-cbind(t(rnorm(9.5*100)), 2.7 + t(rnorm(0.5*100)) )
hist(x[1,])
n<-length(x)
# reorder the sample
...
for (i in 2:nSim){
  ...
  for (t in 1:n){
    probZ1[i,t] <- p[i-1] * exp( -(x[t]-mus[i,1])^2 / 2*sigma2 )
    probZ2[i,t] <- (1-p[i-1]) * exp( -(x[t]-mus[i,2])^2 / 2*sigma2 )
  ...
  p[i]<-rdirichlet(1,c( sum(Z[i,]==0), sum(Z[i,]==1)) )[1]
